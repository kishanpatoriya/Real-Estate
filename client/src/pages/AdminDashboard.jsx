...
export default function AdminDashboard() {
  ...
  const handleConfirmAgentPlan = async (agentEmail, planType) => {
    ...
    try {
      await axios.put(`http://localhost:5000/api/agents/plan/${agentEmail}`, {
      ...
      {viewingProperty && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className="bg-[#0D121D] border border-white/10 rounded-3xl max-w-xl w-full p-8 relative shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <img 
              src={viewingProperty.image && viewingProperty.image.startsWith('http') ? viewingProperty.image : `http://localhost:5000${viewingProperty.image}`} 
